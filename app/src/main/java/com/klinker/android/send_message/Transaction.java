...
public class Transaction {
    ...
    private void sendSmsMessage(String text, String[] addresses, long threadId, int delay) {
        ...
        if (saveMessage) {
            ...
            for (int i = 0; i < addresses.length; i++) {
                ...
                messageUri= SqliteWrapper.insert(context,context.getContentResolver(),Telephony.Sms.CONTENT_URI,values);
                //messageUri = context.getContentResolver().insert(Telephony.Sms.CONTENT_URI, values);
    ...
    private void sendMmsMessage(String text, String[] addresses, Bitmap[] image, String[] imageNames, byte[] media, String mimeType, String subject) {
        ...
        try {
            ...
            if (true){//settings.getWifiMmsFix()) {
                sendMMS(info.bytes);
            } else {
