...
public class ApnDatabase {
    ...
    protected MmsConnection.Apn getLocallyConfiguredMmsConnectionParameters() throws ApnUnavailableException {
        if (TexterPreferences.isUseLocalApnsEnabled(context)) {
            ...
            return new MmsConnection.Apn(mmsc, proxy, port);
        }
    ...
    public MmsConnection.Apn getMmsConnectionParameters(final String mccmnc, final String apn) {
        ...
        try {
            ...
            if (cursor != null && cursor.moveToFirst()) {
                MmsConnection.Apn params = new MmsConnection.Apn(cursor.getString(cursor.getColumnIndexOrThrow(MMSC_COLUMN)),
