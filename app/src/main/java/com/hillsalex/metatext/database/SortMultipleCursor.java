...
public class SortMultipleCursor {
    ...
    public int getCount(){
        int toReturn = 0;
        for (Cursor c : mCursors) toReturn+=c.getCount();
        return toReturn;
    }

    public Pair<Cursor,Integer> next() throws MultiCursorAfterLastException {
        if (isAfterLast())
            throw new MultiCursorAfterLastException();
        if (nextToAdvance!=-1){
            mCursors[nextToAdvance].moveToNext();
        }
        ...
        for (int i=0;i<mCursors.length;i++){
            if (mCursors[i].isAfterLast()) continue;
            switch(columnType){
                case LONG_TYPE:
    ...
    public boolean isAfterLast(){
        ...
        {
            if (!c.isAfterLast()) {
                if (nextToAdvance == i && c.getPosition() >= c.getCount()-1)
                {
                    continue;
                }
                else{
    ...
    public class MultiCursorAfterLastException extends Throwable{
