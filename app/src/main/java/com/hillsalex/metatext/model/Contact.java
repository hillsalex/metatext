...
import com.hillsalex.metatext.util.TelephonyUtil;
...
public class Contact extends Model{
    ...
    private static final String TAG = "metatext.ContactModel";
    ...
    public static String formatNameAndNumber(String name, String number, String numberE164) {
        ...
        if (!TelephonyUtil.isEmailAddress(number)) {
            formattedNumber = TelephonyUtil.formatNumber(number, numberE164,"US");
        }
    ...
    public synchronized void setNumber(String number) {
        if (!TelephonyUtil.isEmailAddress(number)) {
            mNumber = TelephonyUtil.formatNumber(number,mNumberE164,"US");
                    //"US");//MmsApp.getApplication().getCurrentCountryIso());
        } else {
            mNumber = number;
        }
    ...
    public synchronized boolean isEmail() {
        return TelephonyUtil.isEmailAddress(mNumber);
    }
...
//                            }
        ...
        private Contact getContactInfo(Contact c) {
            ...
            } else if (TelephonyUtil.isEmailAddress(c.mNumber)) {
...
//            }
            ...
            String normalizedNumber = TelephonyUtil.normalizeNumber(number);
            ...
            if (!TextUtils.isEmpty(normalizedNumber) && !TextUtils.isEmpty(minMatch)) {
                ...
                String numberE164 = TelephonyUtil.formatNumberToE164(number,"US");
                String selection;
                String[] args;
...
//                                }
        ...
        private Contact internalGet(String numberOrEmail, boolean isMe) {
            synchronized (ContactsCache.this) {
                ...
                final boolean isNotRegularPhoneNumber = isMe || TelephonyUtil.isEmailAddress(numberOrEmail);
        ...
        private void remove(Contact contact) {
            synchronized (ContactsCache.this) {
                ...
                final boolean isNotRegularPhoneNumber = contact.isMe() ||
                        TelephonyUtil.isEmailAddress(number);
                final String key = isNotRegularPhoneNumber ?
