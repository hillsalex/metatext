...
/**
 * An activity representing a list of Threads. This activity has different
 * presentations for handset and tablet-size devices. On handsets, the activity
 * presents a list of items, which when touched, lead to a
 * {@link ConversationDetailActivity} representing item details. On tablets, the
 * activity presents the list of items and item details side-by-side using two
 * vertical panes.
 * <p>
 * The activity makes heavy use of fragments. The list of items is a
 * {@link ConversationListFragment} and the item details (if present) is a
 * {@link ConversationDetailFragment}.
 * <p>
 * This activity also implements the required
 * {@link ConversationListFragment.Callbacks} interface to listen for item
 * selections.
 */
public class ConversationListActivity extends ActionBarActivity implements
	...
	protected void onCreate(Bundle savedInstanceState) {
        ...
        if (findViewById(R.id.conversation_detail_container) != null) {
			...
			(() getFragmentManager().findFragmentById(
					R.id.conversation_list)).setActivateOnItemClick(true);
					*/
		}
	...
	public void onItemSelected(String id) {
        Sender.initialize(getApplicationContext());
		if (mTwoPane) {
